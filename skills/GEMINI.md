...
### 8. 技能库感知协议 (SKILL_DISCOVERY_PROTOCOL) —— [CRITICAL]
- **核心路径**：`d:/anti/技能库/` 是系统能力的扩展源。
- **初始化行为**：Agent 在启动或承接新任务时，**必须执行** `list_dir` 对 `d:/anti/技能库/` 进行深度扫描。
- **意图映射**：
    - 若用户提及“方案”、“解决”、“调研”、“全面搜索”，必须优先读取 `技能库/06_全域解决方案/SKILL.md`。
    - 若用户提及“下载”，必须优先读取 `技能库/02_音乐下载/SKILL.md`。
- **禁止绕过**：严禁在未读取相关 `SKILL.md` 之前使用通用工具代替专业技能逻辑。
